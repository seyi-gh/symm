...
bool WebSocketClient::receive(std::vector<uint8_t>& data) {
    uint8_t header[2];
    if (recv(socket_fd_, header, 2, 0) < 2) return false;

    size_t len = header[1] & 0x7F;
    if (len == 126) {
        uint8_t ext[2];
        if (recv(socket_fd_, ext, 2, 0) < 2) return false;
        len = (ext[0] << 8) | ext[1];
    } else if (len == 127) {
        uint8_t ext[8];
        if (recv(socket_fd_, ext, 8, 0) < 8) return false;
        len = 0;
        for (int i = 0; i < 8; ++i) {
            len = (len << 8) | ext[i];
        }
    ...
    uint8_t mask[4];
    if (recv(socket_fd_, mask, 4, 0) < 4) return false;

    data.resize(len);
    if (recv(socket_fd_, data.data(), len, 0) < (ssize_t)len) {
        return false;
    }
