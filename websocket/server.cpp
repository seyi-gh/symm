...
WebSocketServer::~WebSocketServer() {
  ...
  logger::info("WebSocket server stopped" + std::to_string(port_), __func__);
}
...
bool WebSocketServer::setup_server_socket() {
  ...
  if ((server_fd_ = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
    logger::error("Socket creation failed", __func__);
    return false;
  }
  ...
  if (fcntl(server_fd_, F_SETFL, O_NONBLOCK) < 0) {
    logger::error("Failed to set server socket to non-blocking mode", __func__);
    return false;
  }
  ...
  if (bind(server_fd_, (struct sockaddr*)&address, sizeof(address)) < 0) {
    logger::error("Failed to bind socket to port", __func__);
    return false;
  }

  // Start listening
  if (listen(server_fd_, SOMAXCONN) < 0) {
    logger::error("Failed to listen on socket", __func__);
    return false;
  }
...
void WebSocketServer::run() {
  ...
  logger::info("WebSocket server started on port " + std::to_string(port_), __func__);
...
void WebSocketServer::handle_events() {
  while (running_) {
    ...
    if (num_events < 0) {
      logger::error("epoll_wait failed", __func__);
      break;
    }
      ...
      if (event_fd == server_fd_) {
        ...
        if (client_socket < 0) {
          if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            logger::error("Failed to accept client connection", __func__);
            continue;
          }
        }
        ...
        if (fcntl(client_socket, F_SETFL, O_NONBLOCK) < 0) {
          logger::error("Failed to set client socket to non-blocking mode", __func__);
          close(client_socket);
          continue;
        }
        ...
        logger::info("New client connected: " + std::to_string(client_socket), __func__);
      } else {
...
void WebSocketServer::handle_client_read(int client_socket) {
  ...
  if (len <= 0) {
    if (len < 0) {
      ...
      logger::info("Client disconnected: " + std::to_string(client_socket), __func__);
    }
...
bool WebSocketServer::perform_handshake(int client_socket) {
  ...
  for (int retries = 0; retries < 10; ++retries) { // Retry up to 10 times
    ...
    if (len < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        logger::warn("Socket would block, retrying...", __func__);
      ...
      logger::warn("Client disconnected before handshake", __func__);
      return false;
    }
  ...
  if (len <= 0) {
    logger::error("Failed to read handshake request", __func__);
    return false;
  }
  ...
  if (key_pos == std::string::npos) {
    logger::error("Sec-WebSocket-Key not found in handshake request", __func__);
    return false;
  }
  ...
  logger::info("Generated Sec-WebSocket-Accept: " + accept_key, __func__);
  ...
  logger::info("Sent handshake response", __func__);
  return true;
}

bool WebSocketServer::decode_frame(const char* buffer, long len, std::string& message) {
  if (buffer == nullptr) {
    logger::error("Buffer is null", __func__);
    return false;
  }
  
  if (len < 2) {
    logger::error("Frame too short", __func__);
    return false;
  }
  ...
  if (payload_len == 126) {
    ...
    if (payload_len > static_cast<size_t>(len) - offset - 4) {
      logger::error("Payload length exceeds frame size", __func__);
      return false;
    }
  ...
  if (len < offset + 4 + payload_len) {
    logger::error("Frame too short for payload and masking key", __func__);
    return false;
  }
